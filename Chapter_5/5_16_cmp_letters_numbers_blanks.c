...
int main(int argc, char *argv[]) {
    ...
    if ((nlines = readlines(lineptr, lines, MAXLINES)) >= 0) {
        myqsort(lineptr, 0, nlines - 1, (int (*) (const char *, const char *))(numeric_flag ? numcmp : my_strcmp));
        if (reverse_flag == 1) {
...
int my_strcmp (const char *string1, const char *string2) {
    ...
    do {
        if (direct_flag == 1) {
            while (string1[i] != '\0' && (string1[i] != ' ') && !isalnum(string1[i]))
                i++;
            while (string2[j] != '\0' && (string2[j] != ' ') && !isalnum(string2[j]))
                j++;
            if (string1[i] == '\0' && string1[i] == string2[j])
                return 0;
        }
        
    } while (string1[i++] == string2[j++]);
    return (unsigned char)string1[i - 1] < (unsigned char)string2[j - 1] ? -1 : 1;
}
