...
struct tnode *addmatchtree(struct tnode *, struct tnode *);
int compare(char *, struct tnode *);
struct tnode *talloc(void);
char *mstrdup(char *);
void treeprint(struct tnode *);
int mygetword(char *, int);
int comparematch(struct tnode *, struct tnode *);
void mytolowerstr(char *word);
struct tnode * transfertree(struct tnode *, struct tnode *);
...
struct tnode *addmatchtree(struct tnode *root, struct tnode *node) {
    int cond;

    if (root == NULL) {
        root = talloc();
        root->word = node->word;
        root->match = node->match;
        root->left = root->right = NULL;
    }
    else if (cond = comparematch(root, node) >= 0)
        root->right = addmatchtree(root->right, node);
    else
        root->left = addmatchtree(root->left, node);
    return root;
}

int comparematch(struct tnode *root, struct tnode *node) {
    return (root->match - node->match);
}
...
struct tnode *transfertree(struct tnode *rootto, struct tnode *rootfrom) {
    if (rootfrom != NULL) {
        rootto = addmatchtree(rootto, rootfrom);
        transfertree(rootto, rootfrom->left);
        transfertree(rootto, rootfrom->right);
    }
    return rootto;
}
